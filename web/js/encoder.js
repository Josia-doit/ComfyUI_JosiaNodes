    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        if (nodeData.name === "JosiaEncoder") { 
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                // 绑定开关组件的回调逻辑（保留原有功能，仅规范代码结构）
                const imageRefSwitch = this.widgets.find(w => w.name === "image_reference_switch");
                const mainLatentSwitch = this.widgets.find(w => w.name === "main_latent_mode");
                ...
                // 主图Latent模式开关回调
                if (mainLatentSwitch) {
                    const originalCallback = mainLatentSwitch.callback;
                    mainLatentSwitch.callback = function() {
                        if (originalCallback) originalCallback.apply(this, arguments);
                    };
                }
                // 参考Latent模式开关回调
